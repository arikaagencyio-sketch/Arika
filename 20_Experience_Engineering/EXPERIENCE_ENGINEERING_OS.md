...
**🟢 The runtime gap this department surfaced is now fixed — and this department was the reason it got built.** Wiring EE on 2026-07-03 found that `arika-runtime` had **no join/barrier primitive**: an `EventEmitter` bus can **fan out but never join**, so there was no way to say *"run X only once A and B have both finished."* This department's pipeline is genuinely **parallel** — §4 runs "UI Designer + Motion Director + 3D Director… in parallel" and requires "Creative Director + QA & Performance Reviewer… **both required**" before launch. Four emits were left with **no agent listener**, because wiring them naively would fire the downstream agent two or three times.

**On 2026-07-15 the fix flagged here was built** — `type: join` with `waits_for: [A, B, …]` and `correlate_on` — after Audits & Diagnostics (14) hit the same wall, making it the second department blocked by it. Of the four orphaned emits, **two are now genuinely wired and two remain human by design**:

- **✅ `MOTION_SPEC_READY` + `CAMERA_SPEC_READY`** — the Technical Director now runs on a real barrier: `waits_for: [MOTION_SPEC_READY, CAMERA_SPEC_READY, SCENE_COPY_READY]`, correlated on `project`. It fires **once**, when the last of the three lands — which is what §4's parallel notation always meant. (`SCENE_COPY_READY` implies `UI_SPEC_READY` upstream, since the Copywriter waits on it, so those three are the full set.)
- **⬜ `EXPERIENCE_BRAND_CHECKED`** — still **context, not a trigger**. The Brand Strategist checks an experience against Branding (12)'s confirmed identity; the Storyboard Artist reads that check, it doesn't wait on it. Subscribing the Artist to both `UX_PATTERNS_SET` *and* `EXPERIENCE_BRAND_CHECKED` would still fire it twice, and a barrier would stall storyboarding behind a check that isn't a prerequisite.
- **⬜ `CREATIVE_GATE_PASSED` + `TECHNICAL_GATE_PASSED`** — **both must pass, then a human launches.** This join stays a person on purpose. No agent sits downstream of both gates, because launching a public site is a human act regardless (same shape as Design (19)'s `DELIVERABLE_ASSEMBLED`). The barrier could express it; the department shouldn't.

**Two caveats on the new primitive**, both real:
1. **It awaits a fixed set of distinct event types.** That fits EE exactly (three named spec events, two named gates). It does **not** fit a fan-in whose membership is decided at runtime — see `14_Audits_Diagnostics/AUDITS_DIAGNOSTICS_OS.md` §12, where the scoped sub-audit set varies per engagement and the barrier therefore can't be used.
2. **Joins are in-memory and per-process.** Pending barriers do not survive a runtime restart. Fine for EE, where the spec work is a single session; a blocker for a multi-day workflow.

Recorded here so `operations-state-monitor` (08) reads `EXPERIENCE_BRAND_CHECKED` and the two gate events as **known context/human joins, not broken links** — and no longer reads the Technical Director's inputs as orphaned at all.
...
- 2026-07-15 — **🟢 The join/barrier gap this department surfaced on 2026-07-03 is now built** — `type: join` with `waits_for: [A, B, …]` + `correlate_on`, added to `arika-runtime` after Audits & Diagnostics (14) became the second department blocked by it. **The Technical Director now runs on a real barrier** (`[MOTION_SPEC_READY, CAMERA_SPEC_READY, SCENE_COPY_READY]`, correlated on `project`), firing once when the last lands instead of three times. Orphaned emits drop **4 → 2**: `EXPERIENCE_BRAND_CHECKED` stays **context, not a trigger** (the Storyboard Artist reads the brand check, it doesn't wait on it), and the two launch gates stay a **human join** by design — the barrier could express it, but launching a public site is a human act. Two honest caveats recorded in §12: the barrier awaits a **fixed** set of event types (which fits EE exactly, and does **not** fit 14's runtime-decided sub-audit set), and joins are **in-memory**, so they don't survive a restart. — Claude Code (Opus 4.8)
