...
|---|---|---|---|
| A | **Microsoft 365 is not recorded anywhere** — not in the Tech Stack inventory, not in the DPA's sub-processor register. Three real mailboxes exist (`support@`, `growth@`, `mary.thuo@arikaagency.com`). | `13_Tech_Stack/TECHSTACK_OS.md`, `10_Legal/templates/DPA.md` Annex B | **Not a criticism of the choice.** Email holds the personal data of every client contact, and a mail platform is a sub-processor and a cross-border transfer. A client asking for the sub-processor list must be given a complete one. |
...
| C | **Domain registrar unconfirmed.** `arikaagency.com` is registered but the registrar — and therefore the registrant of record — is not documented. | `13_Tech_Stack/TECHSTACK_OS.md` §9 | For an IP-first company the domain is a core asset. **If it is registered in Mary's personal name it must be assigned to the correct entity** — and per §4 below, that should happen *before* value accrues, not after. |
| D | **Speculative brand work appears in the counsel brief but nowhere in the OS.** The PDF proposes unsolicited redesigns of well-known brands to demonstrate capability. | `The First Entity.pdf` §15 | This is **third-party trademark use**. It is a real, publishable-risk activity with no policy behind it anywhere in the repo. Counsel was asked for a policy — good — but Operations/Content have no rule to follow today. |
