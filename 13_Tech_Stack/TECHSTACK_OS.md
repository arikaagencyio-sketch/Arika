...
| Render | Managed-container cloud **host (L0)** for the Content Distribution Engine — runs self-hosted Postiz + the orchestrator, always-on, flat plan; chosen over Railway (usage-priced) and Coolify/VPS (more hands-on) for a solo owner new to infra | **🟡 Selected 2026-08-02 (owner) — NOT yet provisioned.** No account, no deployed service. `verified_at` to be set on first deploy. |
