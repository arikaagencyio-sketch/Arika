...
| Railway | Managed PaaS **host for the Postiz executor (L0/L6)** — official one-click Postiz template deploys app + Postgres + Redis together (pinned v2.11.3, side-stepping the **Temporal** service Postiz v2.12+ requires), auto-wires secrets, ~$5/mo Hobby plan | **🟡 Selected 2026-08-02 (owner) — NOT yet provisioned.** Chosen over Render for the executor once it emerged Postiz v2.12+ needs a Temporal service Render can't host managed (`docs.postiz.com/configuration/reference`). `verified_at` to be set on first deploy. |
