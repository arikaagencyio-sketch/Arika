...
- **🔴 Email hosting is unidentified, and that is now a live question.** `arikaagency.com` has **three live mailboxes** (`support@`, `growth@`, `mary.thuo@` — observed in ClickUp's real member list, 2026-07-15). **Microsoft 365 was evaluated and rejected on cost (owner, 2026-07-19)** — a per-seat monthly business subscription is not viable pre-revenue — so it is **not** the provider and must **not** be entered in §3 or in the DPA's sub-processor register. **But something is serving those mailboxes.** Whatever it is holds the name and email address of every client contact the agency ever corresponds with, which makes it **a sub-processor and a cross-border transfer**. Identify it before the first client asks for the sub-processor list.
