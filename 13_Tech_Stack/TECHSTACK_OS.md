...
| Self-hosted VPS + Coolify | **Host for the Postiz executor (L0/L6)** — a Kenyan **KVM VPS** (≥4GB RAM, Ubuntu 24.04) paid via **M-Pesa**, running **Coolify** (free, open-source self-hosted PaaS) which deploys Postiz + Postgres + Redis. Candidate providers: Truehost · Novahost · Global Cloud · Hostraha · Lineserve | **🟡 Selected 2026-08-07 (owner) — provider/instance not yet provisioned.** Chosen because Railway rejected the virtual card; Kenyan providers accept M-Pesa. **≥4GB RAM required** (the OOM lesson). `verified_at` on first deploy. |
