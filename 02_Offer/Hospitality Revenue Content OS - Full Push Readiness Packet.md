...
| **PG4 · Before `offer-pricing-floor-analyst`** | ⛔ **Skipped by default** | RD5. A positive pricing run is impossible: no hotel floors, no H-band support, no approved test prices |
| **PG5 · Before Content / Marketing / Sales** | ❌ | Offer output, owner-reviewed · RD6 downstream scope |
...
| Where | Holds |
|---|---|
| **Client folder** (outside every git working tree — RD2) | Pilot name ↔ ID key · observation notes · screenshots (if allowed) · Sector fit record · S10 packet · seed brief · full stdout of every run · owner review notes · downstream hand-off drafts |
...
| 4 | **Offer (02) — pricing advisory** | **Skipped by default** (RD5). Only as an owner-approved negative re-test, expected `insufficient_data` | `arika run offer-pricing-floor-analyst` | A positive floor check — impossible today (§7) |
| 5 | **Content (04) · Marketing (03) · Sales (05)** | **Prepare later hand-offs only after the Offer output is reviewed** (PG5) | **Draft hand-off notes by default** (RD6) — the *angle*, never the artifact (S10 boundary law) | Publication · outreach · CRM lead · Notion write · `sales-lead-qualification` |
...
| **API key rotation still recommended** | Every run uses the key whose value was displayed in a session transcript on 2026-09-13 | ◐ Owner decision RD3 before PG0 | Item 57 |
...
| **R2** | **Sector fit check** (PG1) — no agent | Session reads plugin P1, P2, P4, P9–P11 and checks the OI4 destination's profile, read-only | `R2_sector_fit_record.md` — sector fit, archetype, destination, H-band, anti-ICP, verdict `in scope` / `out of scope`, each with its basis | `<CLIENT>`; verdict only in §11 | Any PG1 stop rule |
...
| **R9** | **Downstream hand-off drafts** (PG5) — no agent by default | Three notes, *angle not artifact*, each labelled pre-audit · Not Quotable: **Content (04)** content-angle brief · **Marketing (03)** market-angle note (carried by hand — no route, item 31k) · **Sales (05)** pre-qualification note (carried by hand — no delivery). *Optional, only if RD6 allows:* `content-intelligence-hub` (`source_signal`) and `marketing-market-intelligence` (`context`) runs — each would **create a new tracked memory log** (`04_Content/_memory/runtime.jsonl`, `03_Marketing/_memory/runtime.jsonl`) | `R9_handoff_content.md` · `R9_handoff_marketing.md` · `R9_handoff_sales.md` | `<CLIENT>` | PG5 not passed · anything would publish, contact the property, create a CRM lead or write to Notion · `sales-lead-qualification` requested (it needs a CRM `lead_id`; no lead is created in this push) |
...
| **RD5** | **Pricing step:** skip, or run a negative re-test? | **Skip** — `_memory/runtime.jsonl` line 5 already proved `insufficient_data`, and a re-run adds a pilot line to the log for no new evidence | PG4 |
| **RD6** | **Downstream scope:** hand-off notes only, or also manual `content-intelligence-hub` / `marketing-market-intelligence` runs? Any Notion or ClickUp write? | **Notes only; no Notion or ClickUp write** | PG5 |
