...
| 34 | Real agency revenue targets — daily, weekly, monthly, quarterly, half-year, yearly | Owner asked for this directly to set the pricing-floor methodology (item 7) and outreach/onboarding capacity against real numbers. **Confirmed absent anywhere in the repo, 2026-06-30** — only an illustrative worked example ($50K/month, explicitly labeled "Example") and a measurement-cadence structure (what to track daily/weekly/monthly, not target values) exist, in `14_Cross_Domain_Synthesis/Agency Revenue Orchestration. Draft 7.md` | Confirmed absent, not just unmigrated | `02_Offer/OFFER_OS.md` §10; `14_Cross_Domain_Synthesis/Agency Revenue Orchestration. Draft 7.md` |
...
| # | Item | Why it matters | Currently | Source file |
|---|---|---|---|---|
| 7 | Real pricing — compute the segmented pricing-floor mean (methodology adopted 2026-06-30, not yet computable) | 7 conflicting pricing sources now exist (5 Offer drafts, Sector's real ICP database, and the first OEOS-engineered offer). Owner's direction: don't pick one as correct — compute a real per-(offer × sector-tier × company-size) mean as an internal "do not discount below" floor, using Sector's Tier 1/2/3 ICP bands for segmentation. **Blocked on**: (a) more real OEOS-engineered offers per segment (only 1 of likely many received so far, and partial), (b) real agency revenue targets (item 34) | Methodology adopted; first real floor calculation pending more data | `02_Offer/OFFER_OS.md` §3, §10 |
