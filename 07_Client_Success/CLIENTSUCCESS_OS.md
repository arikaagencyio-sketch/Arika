...
- 2026-09-14 — **Intake form gap (§10 "concrete intake forms … theory-only") addressed at governance level, owner review required.** `00_Agency_Governance/CLIENT_INTAKE_PROFILE.md` is the universal, stage-gated client intake; its S4 section P maps onto this department's 6-check onboarding diagnostic (Draft 16) and `client-success-onboarding`'s `success_metrics` / `scope_summary`. Hospitality pilot overlay in `02_Offer/`. No agent, CRM field or workflow in this department changed. Tracker item 73. — Claude Code (Opus 5)
