...
Full ground truth in [`contracts/event-catalog.json`](contracts/event-catalog.json), regenerated by grepping every `on:` trigger across all 115 agent files — **not** copied from the prose table in Contract §8, which is where the two unbuilt edges were hiding.

Reality vocabulary: [`AEIT_11`](../00_Agency_Governance/enterprise_architecture/AEIT_11_RUNTIME_TRUTH_STANDARD.md) §2. **Restated 2026-09-13** — this table used `✅ live` / `🔴 dead` until then, sixteen days after the JSON twin had migrated.

| Event | Subscribers | Reality state |
|---|---|---|
| `SECTOR_MAPPED` | content-intelligence-hub (04), offer-orchestrator (02) | **`CONNECTED`**, cross-department |
| `ICP_CLASSIFIED` | sales-lead-qualification (05), clientpartner-partner-sourcing (06) | **`CONNECTED`**, cross-department |
| `PROSPECT_SCORED` | sales-lead-qualification (05), operations-opportunity-filter (08) | **`CONNECTED`**, cross-department |
| `SECTOR_READINESS_SET` | marketing-demand-generation (03) | **`CONNECTED`**, cross-department |
| `CALENDAR_UPDATED` | sector-intelligence-mapper (01) | **`CONNECTED`** — ⚠️ intra-department only |
| `REGULATORY_CHANGE` | sector-readiness-analyst (01) | **`CONNECTED`** — ⚠️ intra-department only |
| `DEMAND_SHIFT` | — | **`DESIGNED`** — retired 2026-08-28 (31d) |
| `COMPRESSION_EVENT` | — | **`DESIGNED`** — retired 2026-08-28 (31d) |
| `COMPETITOR_MOVE` | — | **`DESIGNED`** — retired 2026-08-28 (31d) |

**Zero are `LIVE`.** A verified subscriber is `CONNECTED`; `LIVE` needs a dated execution record, and nothing publishes agent-to-agent.

### 6.1 Two asserted edges that do not exist, in no gap register
