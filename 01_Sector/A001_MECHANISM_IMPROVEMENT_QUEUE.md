...
| O5 | A `reject` from `offer-orchestrator` still **advertised `OFFER_BRIEF_RECEIVED`** — the event `offer-oeos-engineer` subscribes to — because `emits` was static. Observed on the A001 D21 fixture run. Nothing chained only because no path sends agent emits. | Routing logic: an event that could advance a **stopped** sequence (PG3 · R5 · RD7). It describes how the system behaves, not how any market behaves | `arika-runtime/src/executor.ts` (`advertisedEmits`); `OFFER_OS.md` §12; `arika-runtime/DECISIONS.md` | Low | ✅ **Implemented 2026-09-22** — withheld on `reject` only. The other three registry actions keep advertising the event, gated by human review rather than suppression. No publishing added, no registry behaviour changed |
...
- 2026-09-22 — **O5 added and implemented: a rejected brief no longer advertises `OFFER_BRIEF_RECEIVED`.** A **D20-permitted mechanism finding** from the A001 D21 fixture run — routing logic, not market evidence. The orchestrator's static emit meant a `reject` still reported the event that hands a brief to OEOS; the fix withholds it for `reject` only. **No finding about any market, property or buyer is drawn**, and the fix changes no band, gate, ICP rule or registry behaviour. — Claude Code (Opus 5)
- 2026-09-21 — **Correction: this queue's O1 row is NOT stale, contrary to my earlier entry below.** That entry said §5's O1 row *“still reads as open”*. **It does not.** Its final column reads **“✅ Implemented 2026-09-16 — recorded as an evaluation-order rule only”**, and has throughout. The error was mine: I truncated the row when reading it and never saw the status column, then proposed re-doing implemented work on that basis. **No change is needed to §5, and none has been made.** — Claude Code (Opus 5)
- 2026-09-21 — **Correction to the pointer below, and a note on O1’s row.** That entry said **§7’s R2/R3/R5 are what block every runtime test**. **They are not the governing bar:** **D15** forbids *“`arika run`, agent, skill, scheduler or event activity”* for A001 Phase 3, and **D6** keeps the R2 build itself deferred — so **R2 alone could never authorise an A001 agent or skill run**. Corrected in `A001_AGENCY_SYSTEMS_TEST_PLAN.md` v0.2 §3. **Separately:** **O1 was implemented 2026-09-16** (closeout §2 evaluation-order rule + `OFFER_OS.md` §15 pointer), but **§5’s O1 row still reads as open** — that mismatch is what led v0.1 of the plan to propose re-doing it. **The row is left as written**; flagged here for the owner rather than changed unilaterally. — Claude Code (Opus 5)
