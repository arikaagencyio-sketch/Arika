...
**Department:** Sector (01) · **Version:** v0.4 (2026-08-24) · **Status:** Gate 3 — **Phase 1 complete, Phase 2 begun.** `S01`, `S02` and `S03` are live at `.claude/skills/` and have executed against Notion; S04–S12 remain contract-only.
...
**Fix first:** Maasai Mara and Diani are `Level = City`; the `Destination` level exists for exactly them, and Resolution Engine step 1 filters by geography **subtree** — a wrong level returns a wrong signal set silently (F3).
