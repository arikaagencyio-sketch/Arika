...
**Department:** Sector (01) · **Version:** v0.4 (2026-09-13) · **Created:** 2026-08-19
**Status:** **Pass 1 (spec) + Pass 2a applied + the stores populated (2026-08-24/28).** ✅ **`BUILT` and carrying rows:** the §8 colour/type taxonomy (`Signal Type` 16 → 21), the §6 direction model at signal level (`Signal Role`), §7 change-versioning fields, the §10 operating views, and a real Kenya-inbound pilot calendar with §5 timing offsets written. **DB 14 Signal Sources = 33 rows · DB 15 Market Routes = 5 routes**, DB 7 wired to both.

> 🔴 **Corrected 2026-09-13 — this status block was inverted.** It read *"Still design-only because the stores are EMPTY"* and *"**No external source has been registered** — every §12 candidate remains unverified, with no URL recorded."* **Both were false from 2026-08-24**, when S03's first pass registered four sources; two further passes followed on 2026-08-28. The source-registry discipline (§2/§3) and route-level intelligence (§6.1) **do operate** — Gate F's third run scoped origin markets through DB 15 and found Maasai Mara and Diani draw from **completely disjoint** source markets. 🔲 **Genuinely still design-only:** ICS subscription (§3.1) — no registered publisher offers a feed — and the demand-surge rule (§9), which has no `Demand`-type signal to fire on. *The correct line survives and is worth keeping: a built table is not a followed source. It simply stopped being the situation three weeks ago.*
...
- **v0.4 (2026-09-13) — the status block was asserting the opposite of the live state.** It said the stores were empty and *"no external source has been registered"*; **DB 14 had held rows since 2026-08-24 and holds 33**, DB 15 holds 5, and Gate F's third run had already used them to prove that Maasai Mara and Diani draw from disjoint origin markets. The header also read **v0.1** against a **v0.3** changelog — the body had moved three times and the header never did. Corrected, and both defects are now caught by [`contracts/sector_truth_gate.py`](contracts/sector_truth_gate.py) check 4 (header ↔ changelog agreement). **What remains genuinely design-only is now named narrowly:** ICS subscription, because no registered publisher offers a feed, and the §9 demand-surge rule, because no `Demand`-type signal exists to fire it. — Claude Code (Opus 5)
