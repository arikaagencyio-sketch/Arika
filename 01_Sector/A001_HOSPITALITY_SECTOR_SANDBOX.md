...
| **D21** | **A001 TEST_FIXTURE mechanism lane — ONE invocation attempt** | ✅ **ENACTED 2026-09-21 (owner approval in writing).** Authorises **ONE manual invocation attempt** of `offer-orchestrator` through the `TEST_FIXTURE` lane, carrying the exact approved `A001-P07` `seed_brief` (`A001_AGENCY_SYSTEMS_TEST_PLAN.md` §6.4), writing **at most one marked line** to `02_Offer/_memory/sandbox.jsonl`.<br>**Narrow D17 amendment — storage, stated accurately:** that stream is separate and marked, but it sits **inside the git-tracked, auto-synced repository**, so this decision **explicitly permits exactly ONE shared fixture log entry** to be created and committed. It makes **no claim** that A001 stays out of shared storage.<br>**Still barred:** the real runtime stream (`02_Offer/_memory/runtime.jsonl` and every other department’s) · the Sector store · the CRM · **any registry change**, whatever `registry_action` returns · and **use as Offer evidence** or as evidence about any market, property or buyer.<br>**Supersedes D15** for this one attempt only; document-only remains the default for all other A001 work. **Amends D6** to un-defer **T1-5 only** — T1-4 stays deferred, so **no skill run**. **D18 and D20 are unchanged and still bind.**<br>**No automatic retry:** if the call fails **before a log line is written**, the authorisation is **spent**; a retry needs a **fresh owner decision**. **No concurrent invocation.** Authorised whether the attempt succeeds or fails; **expires on that single attempt**.<br>**Confers nothing else:** no Offer eligibility, no registry change, no group ID, no change to `PILOT-H-001` or any PG gate. The owner also approved **verifying the replacement API key by use** as part of this same attempt.<br>⚠️ **File-existence guards a second completed write; it is not proof that only one API attempt occurred.**<br>✅ **SPENT 2026-09-21** — the single attempt was made (exit 0; one `TEST_FIXTURE` line in `02_Offer/_memory/sandbox.jsonl`; `registry_action: reject`; SR-1–SR-3 survived; `runtime.jsonl` byte-unchanged). **No further run is authorised under D21.** 🔴 **The result is a successful stop-rule test, not permission to advance:** `reject` stops the sequence (readiness packet PG3 · R5 · RD7), so the A001-P07 sequence **ends here** and **no `offer-oeos-engineer` continuation is permitted**. Record: `A001_AGENCY_SYSTEMS_TEST_PLAN.md` §6.8 |
...
- 2026-09-22 — **D21's result recorded as a completed stop-rule test.** `offer-orchestrator` returned `reject` on `A001-P07`; the readiness packet's PG3, R5 and RD7 all state that `reject` **stops the sequence**, and RD7's structural continuation applies only to `needs_more_seed_data`. **The A001-P07 sequence ends here**; no OEOS continuation is permitted, and a session suggestion to run one is withdrawn. Mechanism evidence only (D20). No decision other than the D21 row's note changed. — Claude Code (Opus 5)
- 2026-09-21 — **D21 enacted, and its single authorised attempt made and SPENT.** §5.1 carries D21; the lane wrote **one** `TEST_FIXTURE`-marked line to `02_Offer/_memory/sandbox.jsonl` — a **shared, git-tracked, auto-synced** file, which D21 §2 explicitly permits rather than denying. **The real runtime stream is byte-unchanged.** The replacement API key is now **verified by use**. `offer-orchestrator` returned **`reject`**, reproducing **SR-1, SR-2 and SR-3** verbatim and applying the evaluation-order rule, so unit-level passes did **not** become eligibility. **Mechanism evidence only under D20** — no ICP, market, demand, pricing, capacity or real-property claim was produced or may be drawn, and **no registry change is authorised**. D6, D15, D17, D18 and D20 are otherwise untouched, `PILOT-H-001` is untouched, and **no retry is authorised**. — Claude Code (Opus 5)
