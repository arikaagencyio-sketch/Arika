...
**Version:** v1.6 *(the CRM hand-off target verified absent, 2026-09-22; v1.5 recorded SECTOR-SF1's one S10 fixture run — see §6.9)*
**Date:** 2026-09-21
...
- **Corrected later the same day, by an owner-authorised schema read:** the CRM route's four `Lead` tag fields **do not exist live**, so a future S10 run must report the CRM destination as `HANDOFF_FAILURE`, not `not_attempted_fixture`. The attempt record and its artifacts stay as written.
...
- 2026-09-22 — **v1.6. The CRM hand-off target is verified absent.** An owner-authorised read-only schema call found that none of `sector`, `sub_sector`, `icp_tier` or `offer_id` exists on the canonical ClickUp `Lead` list, so S10's CRM route has no target and a future S10 run must report that destination as `HANDOFF_FAILURE`, not `not_attempted_fixture`. A second call confirmed Notion DB 2's six cross-department relations exist and match the contract. SECTOR-SF1's attempt record and its two artifacts stay as written. Target/schema existence only; delivery not tested. **Dated entries below stand as written.** — Claude Code (Opus 5)
