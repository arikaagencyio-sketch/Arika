...
### 🔴 The runtime's 30 schedule triggers have 1 matrix row between them

`arika-runtime` declares **30 `type: schedule` triggers across 29 agent specs** — re-counted from `.claude/agents/*.md` on 2026-09-13 (21 when this section was written on 2026-07-15, with `automation-reliability-monitor` then the 21st). `AUTOMATION_APPROVAL_MATRIX.md` holds **one** real row — for the Creative Pipeline routine.

The matrix rule is unconditional: *"No automation goes live without a row in this matrix."*

**Still not a live breach — and the reason has narrowed to one.** **Manual prompt-agent calls now work:** the Anthropic key was verified by use on 2026-09-13 through five successful manual Offer runs (`13_Tech_Stack/TECHSTACK_OS.md` §3). What keeps the cron triggers **declared, not scheduled** is now only that **no scheduler or daemon has been started — and none has been approved.** Starting the runtime scheduler (`arika-runtime/src/index.ts`) could activate **all 30 declared schedule triggers at once, with no matrix rows, no defined rollback, and no fallback.** **The safety blocker is governance approval and daemon control, not key absence.**

*Was (until 2026-09-13): "The runtime is a local Node process; nobody has booted it as a persistent service, and it has no `ANTHROPIC_API_KEY`. … The moment someone runs `npx tsx src/index.ts` with a key, 21 automations begin firing…"*

That is luck, not compliance. `automation-approval-gate` is instructed to return `blocked` on any request to boot the runtime as a daemon until rows exist. **Rows were not written in this pass** — deciding whether that is 30 rows or one covering row (with a shared rollback: disable the scheduler; and fallback: `arika run <name>` manually) is an owner call (`OWNER_INPUT_NEEDED.md` item 58), and writing 30 rows for triggers with no approved operating history would be documenting an intention as a fact. **The runtime is safe to use manually today; it is not approved to run unattended** — and a key that works for manual Offer calls does not make any of the 29 scheduled agents approved, verified or safe to run unattended.
...
- 2026-09-13 — **§12 re-stated after the Anthropic key was verified: the scheduled triggers' only remaining safeguard is that no scheduler is running.** The old reasoning — *not a live breach because the runtime has no `ANTHROPIC_API_KEY`* — no longer holds: manual prompt-agent calls work (five successful manual Offer runs, 2026-09-13; `13_Tech_Stack/TECHSTACK_OS.md` §3). Replaced with the current truth: the scheduler/daemon is unapproved, the matrix gap stands, starting the scheduler could activate all declared schedule triggers at once, and the blocker is governance approval and daemon control, not key absence. Trigger count re-counted from `.claude/agents/*.md`: **30 across 29 specs** (was 21) — §3, §12 and the cloud-mirror note updated. No agents run; nothing declared live or tested. — Claude Code (Opus 5)
