...
|---|---|---|
| Revenue Growth Advisory | Weekly executive advisory cadence — GTM optimization, revenue diagnostics, scale planning, decision architecture | Offer engineered (`Draft 38`), not yet delivered |
| VIP consulting day | Single-day intensive add-on, $10,000-$35,000/day | Real pricing (Draft 28), not yet productized into a delivery process |

## 4. Workflow Index

| Workflow | Trigger | Steps | Output | Owner | Source |
|---|---|---|---|---|---|
| Revenue Growth Advisory cadence | Signed advisory agreement | Onboarding (stakeholder mapping, data access) → Weekly Advisory Sessions (prep → session → decision-log) → Quarterly Deep Dive → Renewal | Ongoing executive guidance + decision log | Mary Thuo | `Draft 38` (synthesized) |

## 5. Agent Roster

*(placeholder — the offer-engineering file uses functional role labels — Client Partner, Strategy Lead (the advisor) — describing AI-assisted execution functions the owner currently performs solo, not real hires. See `11_HR_People_Ops/HR_OS.md` for when real staffing becomes relevant. Note: this offer's own immutable components explicitly require senior expertise and ban delegation of the advisory content itself — the lowest-delegability offer in the entire catalog, real implication for how this department staffs even after the agency grows beyond solo operation.)*
...
*(placeholder — empty. Note: `Draft 38` surfaced a genuinely new client-risk archetype not covered by the OEOS prompt's standard 7 — "Advisory-as-Validation-Only," a client who uses sessions only to rubber-stamp decisions already made. Worth tracking once real engagements begin.)*
...
None yet — code-based automation not built. The offer-engineering content (`Draft 38`) is the canonical design reference.
