...
| **Host (L0)** | **Render** — managed-container PaaS, always-on, flat plan; chosen over Railway (usage-priced) and Coolify/VPS (more hands-on). |
...
|---|---|---|
| **L0 Foundation** | Host + token vault + the identity spine | Automation (16) · **Render** |
...
- **Phase 0 — Decide & connect.** ✅ platform set / executor / host decided. Remaining: create the 10 accounts (priority order), sign up Render, register connectors with `verified_at`.
...
- **Phase 3 — Executor.** Stand up Postiz on Render; connect LinkedIn first.
