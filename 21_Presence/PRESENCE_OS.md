...
**Update 2026-08-07:** the position-1 platform has its own consolidated dossier — **`21_Presence/LINKEDIN_PRESENCE_OS.md`** (blocking chain, warm-up runway, connection strategy, the reconciled content playbook, and how LinkedIn runs inside the engine). It coordinates; it re-owns nothing.

**Update 2026-08-02:** this intended shape is now fully specified as the **Content Distribution Engine** (`21_Presence/CONTENT_DISTRIBUTION_ENGINE.md`) — the plan-first, two-gate, 12-stage executable workflow across the 10 confirmed platforms, on Postiz (self-hosted via Coolify on a Hostinger KVM 2 VPS, live-deployed 2026-08-07). Owner-ratified design; executor live, platform-connection pending.
