...
**Version:** v0.3-content-migrated
**Last updated:** 2026-06-30
**Status:** Structural skeleton + core governance closed (constitution, RACI, KPI dictionary, CRM schema, automation approval matrix). Content migration complete for all 9 departments with real raw material; 4 departments confirmed to have none. See §9. Remaining work is owner-input (`00_Agency_Governance/OWNER_INPUT_NEEDED.md`) and cross-department reconciliation, not more reading.
...
**What's still open**: a deliberate cross-department reconciliation pass for the scope-blur issues flagged during migration (ClientPartner Acquisition / Sales / Marketing boundary; Client Success qualification/delivery boundaries — see `00_Agency_Governance/OWNER_INPUT_NEEDED.md` and each affected department's §10/§14), and every "needs a real fact from the owner" item rolled up in that same tracker.
