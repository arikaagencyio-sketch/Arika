...
|---|---|---|
| AI Transformation Systems | Organization-wide AI readiness assessment, roadmap, governance framework, training/change management | Offer engineered (`Draft 40`), not yet delivered |
| AI Governance Framework | Risk, data-privacy, fairness review structure required before any production AI deployment — the most rigorous compliance immutable of any offer in the catalog | Designed (`Draft 40`), not yet tested against a real engagement |
...
*(placeholder — the offer-engineering file uses functional role labels — AI Governance Lead, Change Management Lead, Strategy Lead — describing AI-assisted execution functions the owner currently performs solo, not real hires. See `11_HR_People_Ops/HR_OS.md` for when real staffing becomes relevant. Note: governance/risk-framework design is explicitly flagged in `Draft 40` as requiring senior expertise — one of the two hardest-to-delegate deliverable types in the entire Offer catalog, alongside Consulting & Advisory's (15) advisory content.)*
...
*(placeholder — empty. Note: `Draft 40` flags organization-level skepticism/resistance as a distinct risk variant of Automation's individual-level "Doubter" archetype — teams may resist adoption even after leadership buy-in. Carries forward into real incident logging once delivery begins.)*
...
None yet — code-based automation not built. The offer-engineering content (`Draft 40`) is the canonical design reference.
